...
const Widget = ({ widgetData, category, id }) => {
  ...
  return (
    <>
      <div className="flex max-md:justify-center max-md:items-center   gap-y-1 gap-x-3 w-full lg:flex-nowrap lg:overflow-x-auto max-lg:flex-wrap widget bg-[#F0F5FA] ">
        {widgetData.map((data) => (
          <>
            <div
            ...
            >
              <div className="bg-white flex items-center justify-between font-bold text-sm">
                {data.name}
                <img
                  onClick={() => {
                    dispatch(
                  ...
                  className="bg-white size-2 cursor-pointer"
                  src="https://www.pngall.com/wp-content/uploads/4/Cancel-PNG.png"
                />
              </div>
              <p className="bg-white break-words h-auto mt-4 font-semibold w-full  py-2">
                {data.text}
              </p>
            </div>
          </>
